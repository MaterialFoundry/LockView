??? changelog "v2.1.0 - 08-06-2026"

    Compatible with Foundry v13 and v14.

    ### Additions
    * Added similar scene option as the sidebar options for the audio/video dock (auto collapse/expand, blacken background, excluding for autofit).
    * Clone View: Expanded the zoom cloning options: You can clone the actual zoom/scale value (which results in different view depending on display resolution) or clone the current view horizontally, vertically, or automatically (inner/outer view).
    * Clone View: The configuration is now saved between refreshes.
    * Added hotkey to enable/disable viewbox editing (Alt + I by default, can be configured in the settings).
    * If (multiple) modules add tabs to the Scene Config, the tab navigation can get messed up if the tab buttons are too wide. The tab navigation will now be scrollable to prevent this, and autoscroll to the selected tab button (this is applied globally, not just for Lock View).
    ...
    * Autoscaling with the sidebar excluded would not work correctly, especially for the automatic inside/outside fit, this is fixed.
    * Fixed compatibility issue with Daggerheart, where the Lock View scene config would not be rendered.
    * Fixed incompatibility with "Scene Defaults".
    * Clone View warning dialog: if "Do Not Show Again" is ticked, this setting no longer resets on a refresh.
    * Parsing user settings could result in the wrong value due to a mistake in checking is a user is a GM.

    ### Other
    * Hide UI Elements => Camera Feed has been renamed to AV Dock

??? changelog "v2.0.2 - 07-11-2025"
...
??? changelog "v1.4.6 - 15-04-2021"

    ### Fixes
    * Sidebar collapse option didn't work for players

??? changelog "v1.4.5 - 14-04-2021"
